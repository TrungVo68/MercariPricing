...
print("ridgeCV training time: "+str(time.time()-start))
...
print(best1)
print(best2)
print("(Best) RMSL error for RNN + Ridge + RidgeCV on dev set:", rmsle2(Y_dev, Y_dev_preds))
...
submission.to_csv(path+"rnn_ridge_submission_mean.csv", index=False)
...
preds = aggregate_predicts3(rnn_preds, ridgeCV_preds, ridge_preds, 0.6, 0.3)
submission = pd.DataFrame({
        "test_id": test_df.test_id,
        "price": preds.reshape(-1),
})
